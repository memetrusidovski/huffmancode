...
void printDecode(leaf* root, char *s);

int encodeFile(char *cmdFile){
    ...
    while (writeListCurrent != NULL)     
    {
        ...
        else if(writeListCurrent->next == NULL)
...
int decodeFile(char *cmdFile){
    ...
    char *decodeString = (char*) malloc(sizeof(fv));

    printDecode(BST_of_Char, decodeString);
